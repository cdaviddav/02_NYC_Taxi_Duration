...
with tf.device('/gpu:0'):
    ...
    def create_model():
        model = Sequential(activation='relu', init='uniform')
        model.add(Dense(10, input_dim=n_input, kernel_initializer=init, activation=activation))
        model.add(Dense(10, kernel_initializer=init, activation=activation))
        model.add(Dense(10, kernel_initializer=init, activation=activation))
        model.add(Dense(n_classes, kernel_initializer=init))
    
        model.compile(loss='mse', optimizer=optimizers, metrics=['mean_squared_error', 'mean_absolute_error', 'mean_absolute_percentage_error']) # for regression tasks
    
        return model
    
    # Compile model
    
    #model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['acc']) # for classifiaction tasks
    # Fit the model
    model = KerasRegressor (build_fn=create_model, verbose=0, epochs=50,  batch_size=1000)
    #history = model.fit(x_train, y_train, validation_split=0.33, epochs=200, batch_size=1000, verbose=0)
    
    activation = ['tanh', 'sigmoid']
    optimizers = ['adam']
    init = ['uniform']
    param_grid = dict(optimizer=optimizers, init=init, activation=activation)
...
#
#
#print('Minimum validation loss: ' + str(min(history.history['mean_squared_error'])))
#
