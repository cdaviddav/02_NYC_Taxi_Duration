...
param_grid = [{'reduce_dim__n_components': [14],
               'regression__kernel': ['linear', 'rbf'],
               'regression__degree': [2, 3],
               'regression__C': [100, 150],
               'regression__epsilon': [0.1, 0.5, 1],
              }]
...
pipe_best = Pipeline([
    ...
    ('regression', SVR(
        kernel = pipe_best_params['regression__kernel'],
        degree = pipe_best_params['regression__degree'],
        C = pipe_best_params['regression__C'],
        epsilon = pipe_best_params['regression__epsilon'],
    )),
])

print(pipe_best_params['reduce_dim__n_components'])
print(pipe_best_params['regression__kernel'])
print(pipe_best_params['regression__degree'])
print(pipe_best_params['regression__C'])
print(pipe_best_params['regression__epsilon'])

train_errors = evaluate_pipe_best_train(x_train, y_train, pipe_best, 'SVR', log=False, output_file_path=settings.figures)

plot_learning_curve(pipe_best, 'SVR', x_train, y_train, 'r2', output_file_path=settings.figures)
...
evaluate_pipe_best_test(x_test, y_test, pipe_best, 'SVR', log=False, output_file_path=settings.figures)
