...
beta = 0.001 # beta = 0 -> no regularization
logs_path = mydir
...
def nn_layer(input_tensor, input_dim, output_dim, layer_name, output_layer):
  ...
  with tf.name_scope(layer_name):
    ...
    if output_layer is True:
        ...
        activations = tf.nn.dropout(activations, pkeep)
        
    tf.summary.histogram('activations', activations)
    return activations, weights

hidden1, weights_1 = nn_layer(x, n_input, n_hidden_1, 'layer1', output_layer=False)
hidden2, weights_2 = nn_layer(hidden1, n_hidden_1, n_hidden_2, 'layer2', output_layer=False)
hidden3, weights_3 = nn_layer(hidden2, n_hidden_2, n_hidden_3, 'layer3', output_layer=False)
hidden4, weights_4 = nn_layer(hidden3, n_hidden_3, n_hidden_4, 'layer4', output_layer=False)
hidden5, weights_5 = nn_layer(hidden4, n_hidden_4, n_classes, 'layer5', output_layer=True)
...
with tf.name_scope('Loss'):
    #loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y)) # for classification task
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch-1):
            ...
            _, c, a, summary = sess.run([apply_grads, loss, acc, merged_summary_op], feed_dict={x: batch_x, y: batch_y, pkeep: training_dropout})
    ...
    # Test model
    print("Accuracy:", loss.eval({x: x_test, y: y_test, pkeep:1}))
